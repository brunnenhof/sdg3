...
pol_to_expl_CCS_str = [
  "Percent of fossil use you want equip with carbon capture and storage (CCS) at source. This means that you still emit CO2 but it does not get to the atmosphere where it causes warming because you capture it and store it underground.",
...
pol_to_expl_TOW_str = [
  "0 means no wealth tax, 80 means 80% of the accumulated wealth of owners is taxed away each year, 50 half of it. If you think of Elon Musk, the richest person in the world who owns roughly 300 billion USD, 0 would let him keep everything; entering '80' would tax away 240 billion - so he would still have 60 billion(!), and '50' would tax away 150 billion, letting him keep 150 billion - what is fair?",
...
pol_to_expl_REFOREST_str = [
  "Policy to reforest land i.e. plant new trees. 0 means no reforestation, 1 means you increase the forest area by 1 ‰ per yr (that is 1 promille), 3 = you increase the forest area by 3‰ per yr. If you have a forest of 1000 ha, entering 3 gives you 1003 ha next year.",
...
pol_to_expl_FTPEE_str = [
  "Annual percentage increase in energy efficiency; 1% per year is the historical value over the last 40 years. If you enter 1, you will increase efficiency by 10.46 % in year 10",
...
pol_to_expl_LPBsplit_str = [
  "0 means all LBP funding goes to consumption (e.g. child support subsidies for food or energy etc.) 100 means all goes to public investment like infrastructure security etc. NOTE This only has an effect if LPB is NOT set to zero (LPB = Lending from public institutions, like the World Bank)",
...
pol_to_expl_StrUP_str = [
  "In any economy the national income is shared between owners and workers. This policy changes the share going to workers. 1 multiplies the share with 1%, 2 with 2%, etc. If 'normally' workers get 55.00 % of national income, entering 1 would give them 55.55 %",
...
pol_to_expl_FWRP_str = [
  "Here you decide how much the percentage of 'normal' food waste which is 30% is to be reduced. I.e. 100 means no more waste! 50 means waste is reduced by 50 % 0 means waste continues as it always has.",
...
pol_to_expl_ICTR_str = [
  "This policy is an increase in the consumption tax (aka sales tax, value added tax (VAT) etc.) 0 means no increase 10 means an increase by 10 percentage points 5 by 5 percentage points; the money raised goes to general govt revenue. If if was 15%, entering a 10 would raise it to 16.5%",
...
pol_to_expl_XtaxCom_str = [
  "A universal basic dividend is created when a state taxes common goods like fishing rights, mining rights, the right to use airwaves etc. This policy sets this tax as a percent of GDP i.e. 0 = 0 % of GDP i.e. nothing; 5 = 5 % of GDP; 3 = 3 % of GDP The money is transferred to general govt tax revenue.",
...
pol_to_expl_XtaxRateEmp_str = [
  "To support women to reach equality, costs some money esp. to close the pay gender gap. How much do you want to spend as a percent of GDP? 0 means you spend nothing and leave things as they are; 5 means you spend= 5 % of GDP; 3 = 3 % of GDP. Money is transferred to general govt tax revenue",
...
pol_to_expl_XtaxFrac_str = [
  "The percentage of *extra* taxes paid by owners (owners pay 50% of extra taxes even under Too  Little Too Late) I.e. 90 means owners pay 90 % of extra taxes 70 means owners pay 70 % of extra taxes etc. Extra taxes are those for empowerment and to give women equal pay.",
...
sdgvarID_to_subtitle_34_str = [
  "Donor and off-balance sheet investment share (% of GDP) See the glossary (under Help) for an explanation of 'off-balance sheet financing'",
  "Anteil der Geber- und außerbilanziellen Investitionen (in % des BIP) Eine Erläuterung des Begriffs <außerbilanzielle Finanzierung> finden Sie im Glossar (unter Hilfe).",
  "Anteil der Geber- und außerbilanziellen Investitionen (in % des BIP) Eine Erläuterung des Begriffs <außerbilanzielle Finanzierung> finden Sie im Glossar (unter Hilfe).",
  "Part des dons et des investissements hors bilan (% du PIB) Voir le glossaire (sous Aide) pour une explication du terme « financement hors bilan ».",
  "Donor- og ikke-balanseførte investeringsandeler (% av BNP) Se ordlisten (under Hjelp) for en forklaring av «ikke-balanseført finansiering».",
  "_last_"
]
...
msg_gm_board_info_str = [
    "If you do not have enough **human** players for all the roles. click on **all** the regions where the app will make automatic random policy decisions, biased twoards *Giant Leap*",
...
gm_card_wait_1_info_tx_str = [
    "All roles have been set up now. Tell your players to log in **now**, using the Game ID shown above. \nAsk them to look at the state of their region for last 35 years and discuss their decisions to improve the lives of their people. \n\nCheck repeatedly if all your players have logged in by clicking the **Check LogIn** button. \n\nOnce all logged in, you will see a checkbox to accept the players decisions for the next round. Before you check the box right away, think if you want to do a debrief / discussion first, or if you want to continue the game at a later time, i.e. after lunch, next week, during the next teaching period, etc. ",
    "Alle Rollen sind nun eingerichtet. Bitten Sie Ihre Spieler:innen, sich **jetzt** mit der oben angegebenen Spiel-ID anzumelden. \nWeisen Sie sie an, sich die Lage ihrer Region in den letzten 35 Jahren anzusehen und ihre Entscheidungen zur Verbesserung der Lebensbedingungen ihrer Bevölkerung zu diskutieren. \n\nÜberprüfen Sie wiederholt, ob sich alle Ihre Spieler angemeldet haben, indem Sie auf die Schaltfläche **Anmeldung überprüfen** klicken. \n\nSobald alle eingeloggt sind, wird ein Kontrollkästchen angezeigt, mit dem Sie die Entscheidungen der Spieler für die nächste Runde akzeptieren können. Bevor Sie das Kästchen sofort anklicken, überlegen Sie, ob Sie zuerst eine Nachbesprechung/Diskussion durchführen möchten oder ob Sie das Spiel zu einem späteren Zeitpunkt fortsetzen möchten, z. B. nach dem Mittagessen, nächste Woche, während der nächsten Unterrichtsstunde usw.",
    "Alle Rollen sind nun eingerichtet. Bitten Sie Ihre Spieler:innen, sich **jetzt** mit der oben angegebenen Spiel-ID anzumelden. \nWeisen Sie sie an, sich die Lage ihrer Region in den letzten 35 Jahren anzusehen und ihre Entscheidungen zur Verbesserung der Lebensbedingungen ihrer Bevölkerung zu diskutieren. \n\nÜberprüfen Sie wiederholt, ob sich alle Ihre Spieler angemeldet haben, indem Sie auf die Schaltfläche **Anmeldung überprüfen** klicken. \n\nSobald alle eingeloggt sind, wird ein Kontrollkästchen angezeigt, mit dem Sie die Entscheidungen der Spieler für die nächste Runde akzeptieren können. Bevor Sie das Kästchen sofort anklicken, überlegen Sie, ob Sie zuerst eine Nachbesprechung/Diskussion durchführen möchten oder ob Sie das Spiel zu einem späteren Zeitpunkt fortsetzen möchten, z. B. nach dem Mittagessen, nächste Woche, während der nächsten Unterrichtsstunde usw.",
    "Tous les rôles ont été configurés. Demandez à vos joueurs de se connecter **maintenant** à l'aide de l'identifiant de jeu indiqué ci-dessus. \nDemandez-leur d'examiner la situation de leur région au cours des 35 dernières années et de discuter de leurs décisions visant à améliorer la vie de leur population. \n\nVérifiez régulièrement que tous vos joueurs se sont connectés en cliquant sur le bouton **Vérifier la connexion**. \n\nUne fois que tous les joueurs sont connectés, vous verrez apparaître la case à cocher pour accepter les décisions des joueurs pour le prochain tour. Avant de cocher la case immédiatement, demandez-vous si vous souhaitez d'abord faire un débriefing/une discussion, ou si vous préférez poursuivre le jeu plus tard, par exemple après le déjeuner, la semaine prochaine, pendant la prochaine période d'enseignement, etc.",
    "Alle roller er nå opprettet. Be spillerne logge inn **nå** ved å bruke spill-ID-en som er vist ovenfor. \nBe dem se på tilstanden i regionen deres de siste 35 årene og diskutere beslutningene de har tatt for å forbedre livene til innbyggerne. \n\nSjekk gjentatte ganger om alle spillerne har logget inn ved å klikke på **Sjekk innlogging**-knappen. \n\nNår alle har logget seg på, vil du se avmerkingsboksen for å godta spillernes beslutninger for neste runde. Før du merker av i boksen, bør du vurdere om du først vil ha en debriefing/diskusjon, eller om du vil fortsette spillet senere, f.eks. etter lunsj, neste uke, i neste undervisningstime osv.",
    "_last_"
]
gm_checkbox_open_tx = [
    "Tell your players to look at the state of their region for last 35 years and to discuss in their team, and across teams, their decisions to improve the lives of their people.\n\nBelow is a checkbox to accept the players decisions for the next round. But before you check the box right away, think if you want to do a discussion/debrief first, or if you want to continue the game at a later time, i.e. after lunch, next week, during the next teaching period, etc. If so, wait to check the box until you are ready to continue-",
    "Bitten Sie Ihre Spieler:innen, sich die Entwicklung ihrer Region in den letzten 35 Jahren anzusehen und in ihrem Team sowie teamübergreifend ihre Entscheidungen zur Verbesserung der Lebensbedingungen ihrer Bevölkerung zu diskutieren. \n\n Weiter unten finden Sie ein Kontrollkästchen, um die Entscheidungen der Spieler für die nächste Runde zu akzeptieren. Bevor Sie das Kästchen jedoch sofort anklicken, überlegen Sie, ob Sie zunächst eine Nachbesprechung/Diskussion durchführen möchten oder ob Sie das Spiel zu einem späteren Zeitpunkt fortsetzen möchten, z. B. nach dem Mittagessen, nächste Woche, während der nächsten Unterrichtsstunde etc. Wenn ja, warten Sie mit dem Aktivieren des Kontrollkästchens, bis Sie bereit sind, fortzufahren.",
    "Bitten Sie Ihre Spieler:innen, sich die Entwicklung ihrer Region in den letzten 35 Jahren anzusehen und in ihrem Team sowie teamübergreifend ihre Entscheidungen zur Verbesserung der Lebensbedingungen ihrer Bevölkerung zu diskutieren. \n\n Weiter unten finden Sie ein Kontrollkästchen, um die Entscheidungen der Spieler für die nächste Runde zu akzeptieren. Bevor Sie das Kästchen jedoch sofort anklicken, überlegen Sie, ob Sie zunächst eine Nachbesprechung/Diskussion durchführen möchten oder ob Sie das Spiel zu einem späteren Zeitpunkt fortsetzen möchten, z. B. nach dem Mittagessen, nächste Woche, während der nächsten Unterrichtsstunde etc. Wenn ja, warten Sie mit dem Aktivieren des Kontrollkästchens, bis Sie bereit sind, fortzufahren.",
    "Demandez à vos joueurs d'examiner la situation de leur région au cours des 35 dernières années et de discuter au sein de leur équipe, puis entre équipes, des décisions qu'ils ont prises pour améliorer la vie de leur population. \n\nVous trouverez ci-dessous une case à cocher pour accepter les décisions des joueurs pour le prochain tour. Mais avant de cocher la case immédiatement, demandez-vous si vous souhaitez d'abord faire un débriefing/une discussion, ou si vous préférez poursuivre le jeu plus tard, par exemple après le déjeuner, la semaine prochaine, pendant la prochaine période d'enseignement, etc. Si oui, attendez d'être prêt à continuer avant de cocher la case.",
    "Be spillerne se på tilstanden i regionen deres de siste 35 årene og diskutere i teamet sitt og på tvers av teamene hvilke beslutninger de vil ta for å forbedre livene til innbyggerne. \n\nNedenfor er det en avkrysningsboks for å godta spillernes beslutninger for neste runde. Men før du krysser av i boksen med en gang, bør du vurdere om du først vil ha en debriefing/diskusjon, eller om du vil fortsette spillet senere, for eksempel etter lunsj, neste uke, i neste undervisningstime osv. Hvis ja, vent med å merke av i boksen til du er klar til å fortsette.",
    "_last_"
]
gm_checkbox_open40_tx = [
    "Tell your players to look at the state of their region, especially for 2025 to 2040. Let them discuss in their team, and across teams, if things developed as they expected and hoped for. Then, ask them to come up with their decisions to improve the lives of their fellow citizens for the period 2040 to 2060 - the lives of their children. \n\nFor you, there is a checkbox to accept the players' decisions for the next round. But before you check the box right away, think if you want to do a debrief / discussion first, or if you want to continue the game at a later time, i.e. after lunch, next week, during the next teaching period, etc.",
    "Bitten Sie Ihre Spieler:innen, sich die Lage ihrer Region anzusehen, insbesondere für den Zeitraum von 2025 bis 2040. Lassen Sie sie in ihrem Team und teamübergreifend diskutieren, ob sich die Dinge so entwickelt haben, wie sie es erwartet und erhofft hatten. Bitten Sie sie dann, Entscheidungen zu treffen, um das Leben ihrer Mitbürger:innen für den Zeitraum von 2040 bis 2060 zu verbessern – also das Leben ihrer Kinder. \n\nFür Sie gibt es ein Kontrollkästchen, um die Entscheidungen der Spieler:innen für die nächste Runde zu akzeptieren. Bevor Sie das Kästchen jedoch sofort anklicken, überlegen Sie, ob Sie zunächst eine Nachbesprechung/Diskussion durchführen möchten oder ob Sie das Spiel zu einem späteren Zeitpunkt fortsetzen möchten, z. B. nach dem Mittagessen, nächste Woche, während der nächsten Unterrichtsstunde usw.",
    "Bitte Deine Spieler:innen, sich die Lage ihrer Region anzusehen, insbesondere für den Zeitraum von 2025 bis 2040. Lass sie in ihrem Team und teamübergreifend diskutieren, ob sich die Dinge so entwickelt haben, wie sie es erwartet und erhofft hatten. Bitte sie dann, Entscheidungen zu treffen, um das Leben ihrer Mitbürger:innen für den Zeitraum von 2040 bis 2060 zu verbessern – also das Leben ihrer Kinder. \n\nFür Dich gibt es ein Kontrollkästchen, um die Entscheidungen der Spieler:innen für die nächste Runde zu akzeptieren. Bevor Du das Kästchen jedoch sofort anklickst, überlege Dir, ob Du zunächst eine Nachbesprechung/Diskussion durchführen möchtest oder ob Du das Spiel zu einem späteren Zeitpunkt fortsetzen möchtest, z. B. nach dem Mittagessen, nächste Woche, während der nächsten Unterrichtsstunde usw.",
    "Demandez à vos joueurs d'examiner la situation de leur région, en particulier pour la période 2025-2040. Laissez-les discuter au sein de leur équipe et entre équipes pour déterminer si les choses ont évolué comme ils l'avaient prévu et espéré. Demandez-leur ensuite de prendre des décisions visant à améliorer la vie de leurs concitoyens pour la période 2040-2060, c'est-à-dire la vie de leurs enfants. \n\nVous disposez d'une case à cocher pour accepter les décisions des joueurs pour le prochain tour. Mais avant de cocher cette case, demandez-vous si vous souhaitez d'abord faire un débriefing/une discussion, ou si vous préférez poursuivre le jeu plus tard, par exemple après le déjeuner, la semaine prochaine, pendant la prochaine période d'enseignement, etc.",
    "Be spillerne se på tilstanden i regionen deres, spesielt for perioden 2025 til 2040. La dem diskutere i teamet sitt og på tvers av teamene om ting utviklet seg som de forventet og håpet. Be dem deretter komme med beslutninger for å forbedre livene til medborgerne sine for perioden 2040 til 2060 – livene til barna deres. \n\nFor deg er det en avkrysningsboks for å godta spillernes beslutninger for neste runde. Men før du krysser av i boksen med en gang, bør du tenke gjennom om du først vil ha en debriefing/diskusjon, eller om du vil fortsette spillet senere, for eksempel etter lunsj, neste uke, i neste undervisningsperiode osv.",
    "_last_"
]
gm_checkbox_open60_tx = [
    "Tell your players to look at the state of their region, especially for 2040 to 2060. Let them discuss in their team, and across teams, if things developed as they expected and hoped for. Then, ask them to come up with their decisions to improve the lives of their fellow citizens for the period 2060 to 2100 - the lives of their grand-children. \n\nFor you, there is a checkbox to accept the players' decisions for the next round. But before you check the box right away, think if you want to do a debrief / discussion first, or if you want to continue the game at a later time, i.e. after lunch, next week, during the next teaching period, etc.",
    "Bitten Sie Ihre Spieler:innen, sich die Lage ihrer Region anzusehen, insbesondere für den Zeitraum von 2040 bis 2060. Lassen Sie sie in ihrem Team und teamübergreifend diskutieren, ob sich die Dinge so entwickelt haben, wie sie es erwartet und erhofft hatten. Bitten Sie sie dann, Entscheidungen zu treffen, um das Leben ihrer Mitbürger:innen für den Zeitraum von 2060 bis 2100 zu verbessern – also das Leben ihrer Enkel. \n\nFür Sie gibt es ein Kontrollkästchen, um die Entscheidungen der Spieler:innen für die nächste Runde zu akzeptieren. Bevor Sie das Kästchen jedoch sofort anklicken, überlegen Sie, ob Sie zunächst eine Nachbesprechung/Diskussion durchführen möchten oder ob Sie das Spiel zu einem späteren Zeitpunkt fortsetzen möchten, z. B. nach dem Mittagessen, nächste Woche, während der nächsten Unterrichtsstunde usw.",
    "Bitte Deine Spieler:innen, sich die Lage ihrer Region anzusehen, insbesondere für den Zeitraum von 2040 bis 2060. Lass sie in ihrem Team und teamübergreifend diskutieren, ob sich die Dinge so entwickelt haben, wie sie es erwartet und erhofft hatten. Bitte sie dann, Entscheidungen zu treffen, um das Leben ihrer Mitbürger:innen für den Zeitraum von 2060 bis 2100 zu verbessern – also das Leben ihrer Enkel. \n\nFür Dich gibt es ein Kontrollkästchen, um die Entscheidungen der Spieler:innen für die nächste Runde zu akzeptieren. Bevor Du das Kästchen jedoch sofort anklickst, überlege Dir, ob Du zunächst eine Nachbesprechung/Diskussion durchführen möchtest oder ob Du das Spiel zu einem späteren Zeitpunkt fortsetzen möchtest, z. B. nach dem Mittagessen, nächste Woche, während der nächsten Unterrichtsstunde usw.",
    "Demandez à vos joueurs d'examiner la situation de leur région, en particulier pour la période 2040-2060. Laissez-les discuter au sein de leur équipe et entre équipes pour déterminer si les choses ont évolué comme ils l'avaient prévu et espéré. Demandez-leur ensuite de prendre des décisions visant à améliorer la vie de leurs concitoyens pour la période 2060-2100, c'est-à-dire la vie de leurs petits-enfants. \n\nVous disposez d'une case à cocher pour accepter les décisions des joueurs pour le prochain tour. Mais avant de cocher cette case, demandez-vous si vous souhaitez d'abord faire un débriefing/une discussion, ou si vous préférez poursuivre le jeu plus tard, par exemple après le déjeuner, la semaine prochaine, pendant la prochaine période d'enseignement, etc.",
    "Be spillerne se på tilstanden i regionen deres, spesielt for perioden 2040 til 2060. La dem diskutere i teamet sitt og på tvers av teamene om ting utviklet seg som de forventet og håpet. Be dem deretter komme med beslutninger for å forbedre livene til medborgerne sine for perioden 2060 til 2100 – livene til barnebarn deres. \n\nFor deg er det en avkrysningsboks for å godta spillernes beslutninger for neste runde. Men før du krysser av i boksen med en gang, bør du tenke gjennom om du først vil ha en debriefing/diskusjon, eller om du vil fortsette spillet senere, for eksempel etter lunsj, neste uke, i neste undervisningsperiode osv.",
    "_last_"
]
gm_checkbox_open21_tx = [
    "Tell your players to look at the state of their region, especially for 2060 to 2100. Let them discuss in their team, and across teams, if things developed as they expected and hoped for. Are they content? Their citizens? Is the planet still inhabitable? Did they create a good future? A bad one? What went wrong? What went right? What would they do differently next time? In real life, who decides for their, their children's, their grand-children's future? Do you and your players trust whoever they are to make the right decisions?",
    "Bitten Sie Ihre Spieler:innen, sich die Lage ihrer Region anzusehen, insbesondere für den Zeitraum von 2060 bis 2100. Lassen Sie sie in ihrem Team und teamübergreifend diskutieren, ob sich die Dinge so entwickelt haben, wie sie es erwartet und erhofft hatten. Sind sie zufrieden? Ihre Bürger:innen? Ist der Planet noch bewohnbar? Haben sie eine gute Zukunft geschaffen? Eine schlechte? Was ist schiefgelaufen? Was ist gut gelaufen? Was würden sie beim nächsten Mal anders machen? Wer entscheidet im wirklichen Leben über ihre Zukunft, die Zukunft ihrer Kinder und Enkelkinder? Vertrauen sie darauf, dass diese Personen die richtigen Entscheidungen treffen?",
    "Bitte Deine Spieler:innen, sich die Lage ihrer Region anzusehen, insbesondere für den Zeitraum von 2060 bis 2100. Lass sie in ihrem Team und teamübergreifend diskutieren, ob sich die Dinge so entwickelt haben, wie sie es erwartet und erhofft hatten. Sind sie zufrieden? Ihre Bürger:innen? Ist der Planet noch bewohnbar? Haben sie eine gute Zukunft geschaffen? Eine schlechte? Was ist schiefgelaufen? Was ist gut gelaufen? Was würden sie beim nächsten Mal anders machen? Wer entscheidet im wirklichen Leben über ihre Zukunft, die Zukunft ihrer Kinder und Enkelkinder? Vertrauen sie darauf, dass diese Leute die richtigen Entscheidungen treffen?",
    "Demandez à vos joueurs d'examiner la situation de leur région, en particulier pour la période 2060-2100. Laissez-les discuter au sein de leur équipe et entre équipes pour déterminer si les choses ont évolué comme ils l'avaient prévu et espéré. Sont-ils satisfaits ? Leurs citoyens ? La planète est-elle toujours habitable ? Ont-ils créé un avenir meilleur ? Ou pire ? Qu'est-ce qui a mal tourné ? Qu'est-ce qui a bien tourné ? Que feraient-ils différemment la prochaine fois ? Dans la vie réelle, qui décide de leur avenir, de celui de leurs enfants et de leurs petits-enfants ? Leur font-ils confiance pour prendre les bonnes décisions ?",
    "Be spillerne se på tilstanden i regionen deres, spesielt for perioden 2060 til 2100. La dem diskutere i teamet sitt og på tvers av teamene om ting utviklet seg som de forventet og håpet. Er de fornøyde? Er innbyggerne deres fornøyde? Er planeten fortsatt beboelig? Har de skapt en god fremtid? En dårlig fremtid? Hva gikk galt? Hva gikk bra? Hva ville de gjort annerledes neste gang? Hvem bestemmer i virkeligheten over deres, deres barns og deres barnebarns fremtid? Stoler de på at de tar de riktige beslutningene?",
    "_last_"
]
