...
login_str_gm = [
  "The game can be interrupted, by accident or on purpose. In order to continue playing the game after any interruption, you need to register so that you can log in and we can show you the exact page you were on before any interruption.\n\nYour registration e-mail does not have to be your real one. It can be a fantasy address like p1@def.ghi - but the @ sign and the dot must be included. \n\nPlease note: if you do enter a fantasy address, you cannot recover a forgotten password, you can still login, though! You can choose whatever password you want, even nothing at all! But remember well what you enter - after any interruption you have to enter both e-mail and password in exactly the same way again.",
  "Das Spiel kann versehentlich oder absichtlich unterbrochen werden. Um das Spiel nach einer Unterbrechung fortzusetzen, müssen Sie sich registrieren, damit Sie sich anmelden können und wir Ihnen genau die Seite anzeigen können, auf der Sie sich vor der Unterbrechung befanden. \n\nIhre Registrierungs-E-Mail-Adresse muss nicht Ihre echte E-Mail-Adresse sein. Es kann sich um eine Fantasieadresse wie p1@def.ghi handeln – das @-Zeichen und der Punkt müssen jedoch enthalten sein. \n\nBitte beachten Sie: Wenn Sie eine Fantasieadresse eingeben, können Sie ein vergessenes Passwort nicht wiederherstellen, aber Sie können sich trotzdem einloggen! Sie können ein beliebiges Passwort wählen, sogar gar keines! Aber merken Sie sich gut, was Sie eingeben – nach einer Unterbrechung müssen Sie sowohl Ihre E-Mail-Adresse als auch Ihr Passwort wieder genau so eingeben. \n\nFür die Registrierung und Anmeldung verwenden wir die integrierten Funktionen von anvil.works. Die Eingabeaufforderungen sind auf Englisch und wir haben noch nicht herausgefunden, wie wir sie lokalisieren können – sorry.",
  "Das Spiel kann versehentlich oder absichtlich unterbrochen werden. Um das Spiel nach einer Unterbrechung fortzusetzen, müssen Sie sich registrieren, damit Sie sich anmelden können und wir Ihnen genau die Seite anzeigen können, auf der Sie sich vor der Unterbrechung befanden. \n\nIhre Registrierungs-E-Mail-Adresse muss nicht Ihre echte E-Mail-Adresse sein. Es kann sich um eine Fantasieadresse wie p1@def.ghi handeln – das @-Zeichen und der Punkt müssen jedoch enthalten sein. \n\nBitte beachten Sie: Wenn Sie eine Fantasieadresse eingeben, können Sie ein vergessenes Passwort nicht wiederherstellen, aber Sie können sich trotzdem einloggen! Sie können ein beliebiges Passwort wählen, sogar gar keines! Aber merken Sie sich gut, was Sie eingeben – nach einer Unterbrechung müssen Sie sowohl Ihre E-Mail-Adresse als auch Ihr Passwort wieder genau so eingeben. \n\nFür die Registrierung und Anmeldung verwenden wir die integrierten Funktionen von anvil.works. Die Eingabeaufforderungen sind auf Englisch und wir haben noch nicht herausgefunden, wie wir sie lokalisieren können – sorry.",
  "Le jeu peut être interrompu, accidentellement ou intentionnellement. Afin de pouvoir continuer à jouer après une interruption, vous devez vous inscrire afin de pouvoir vous connecter et que nous puissions vous rediriger vers la page exacte où vous vous trouviez avant l'interruption. \n\nVotre adresse e-mail d'inscription ne doit pas nécessairement être votre adresse réelle. Il peut s'agir d'une adresse fictive telle que p1@def.ghi, mais le signe @ et le point doivent être inclus. \n\nRemarque: si vous entrez une adresse fictive, vous ne pourrez pas récupérer un mot de passe oublié, mais vous pourrez tout de même vous connecter ! Vous pouvez choisir le mot de passe que vous voulez, même aucun ! Mais souvenez-vous bien de ce que vous entrez : après toute interruption, vous devrez saisir à nouveau votre adresse e-mail et votre mot de passe exactement de la même manière. Pour l'inscription et la connexion, nous utilisons les fonctionnalités intégrées d'anvil.works. Leurs invites sont en anglais et nous n'avons pas encore trouvé comment les localiser, désolé.",
  "Spillet kan bli avbrutt ved et uhell eller med vilje. For å fortsette spillet etter et avbrudd må du registrere deg, slik at du kan logge inn og vi kan vise deg nøyaktig den siden du var på før avbruddet. \n\nDin registrerings-e-postadresse trenger ikke å være din ekte e-postadresse. Det kan være en fantasiadresse som p1@def.ghi – men @-tegnet og punktumet må være med. \n\nVær oppmerksom på: Hvis du oppgir en fantasiadresse, kan du ikke gjenopprette et glemt passord, men du kan fortsatt logge inn! Du kan velge hvilket som helst passord, eller ikke ha noe passord i det hele tatt! Men husk godt hva du skriver inn – etter en avbrutt må du skrive inn både e-postadressen og passordet ditt på nøyaktig samme måte. \n\nFor registrering og pålogging bruker vi de integrerte funksjonene i anvil.works. Innspillene er på engelsk, og vi har ennå ikke funnet ut hvordan vi kan lokalisere dem – beklager.",
  "_last_"
]
login_str_p = [
  "",
...
lang_info_str = [
  "Welcome to the ... game. Thank you for taking the time to playfully explore the connections between poverty, inequality, empowerment, food & agriculture and energy. It's complex, but not incomprehensible. On the contrary, with your fellow players you can intervene three times in the course of events. To shape your future, the future of your children and the future of your grandchildren in a humane way. You will find out exactly how in the course of the game. \n**But first, the language.** If English is sufficient, simply click on the *Let's go* button, otherwise select the desired language from the drop-down menu. Over time we will add more and more languages, if you want to help with the translation, let us know. \ne-mail: lang at blue minus way dot net",
  "Willkommen zum ... Spiel. Schön, dass Sie sich die Zeit nehmen, spielerisch die Zusammenhänge von Armut, Ungleichheit, Befähigung, Nahrung & Landwirtschaft und Energie zu erkunden. Es ist komplex, aber nicht unverständlich. Im Gegenteil, mit Ihren Mitspieler:innen können Sie dreimal in den Lauf der Dinge eingreifen. Um Ihre Zukunft, die Zukunft Ihrer Kinder, und die Zukunft Ihrer Enkel menschenwürdig zu gestalten. Wie genau, erfahren Sie im Laufe des Spiels. \n**Aber zuerst die Sprache.** Wenn formelles Deutsch ausreicht, klicken Sie einfach auf die Schaltfläche *Los gehts*, andernfalls wählen Sie aus dem Dropdown-Menü die gewünschte Sprache. Mit der Zeit fügen wir mehr und mehr Sprachen hinzu, wenn Sie bei der Übersetzung helfen wollen, lassen Sie es uns wissen. \ne-mail: lang at blue minus way dot net",
  "Willkommen zum ... Spiel. Schön, dass Du Dir Zeit nimmst, spielerisch die Zusammenhänge von Armut, Ungleichheit, Befähigung, Nahrung & Landwirtschaft und Energie zu erkunden. Es ist komplex, aber nicht unverständlich. Im Gegenteil, mit Deinen Mitspieler:innen kannst Du dreimal in den Lauf der Dinge eingreifen. Um Deine Zukunft, die Zukunft Deiner Kinder, und die Zukunft Deiner Enkel menschenwürdig zu gestalten. Wie genau, erfährst Du im Laufe des Spiels. \n**Aber zuerst die Sprache.** Wenn informelles Deutsch ausreicht, klicke einfach auf die Schaltfläche *Los gehts*, andernfalls wähle aus dem Dropdown-Menü die gewünschte Sprache. Mit der Zeit fügen wir mehr und mehr Sprachen hinzu, wenn Du bei der Übersetzung helfen willst, lass es uns wissen. \ne-mail: lang at blue minus way dot net",
  "Bienvenue au ... Jeu de société. Nous sommes heureux que vous preniez le temps d'explorer de manière ludique les liens entre pauvreté, inégalité, autonomisation, alimentation & agriculture et énergie. C'est complexe, mais pas incompréhensible. Au contraire, avec vos coéquipiers, vous pouvez intervenir trois fois dans le cours des choses. Pour construire votre avenir, l'avenir de vos enfants et l'avenir de vos petits-enfants dans la dignité. Vous apprendrez comment exactement au cours du jeu. \n**Mais d'abord la langue.** Si le français formel suffit, il vous suffit de cliquer sur le bouton *Allons-y*, sinon choisissez la langue souhaitée dans le menu déroulant. Au fil du temps, nous ajoutons de plus en plus de langues, si vous souhaitez aider à la traduction, faites-le nous savoir. \ne-mail: lang at blue minus way dot net",
  "Velkommen til... spillet. Takk for at du tar deg tid til å utforske sammenhengene mellom fattigdom, ulikhet, empowerment, mat og landbruk og energi på en leken måte. Det er komplekst, men ikke uforståelig. Tvert imot, sammen med dine medspillere kan du gripe inn tre ganger i hendelsesforløpet. For å forme din egen, dine barns og dine barnebarns fremtid på en human måte. Du vil finne ut nøyaktig hvordan i løpet av spillet. \n**Men først språket** Hvis bokmål er tilstrekkelig, klikker du bare på *La oss gå* knappen, ellers velger du ønsket språk fra rullegardinmenyen . Over tid vil vi legge til flere og flere språk, så hvis du vil hjelpe til med oversettelsen, gi oss beskjed. \ne-post: lang at blue minus way dot net",
  "_last_"
    ]
lang_lets_go_tx = [
  "Let's go",
